...
function proxyPath(type, absoluteUrl) {
  return `/hls/${type}?url=${encodeURIComponent(absoluteUrl)}`;
}
...
function rewritePlaylist(content, baseUrl) {
  return content
    ...
    .map((line) => {
      ...
      if (trimmed.includes(".m3u8")) return proxyPath("manifest", absolute);
      if (trimmed.endsWith(".ts")) return proxyPath("segment", absolute);
      return line;
    })
    .join("\n");
}
...
function transcodeSegment(url) {
  ...
  const job = new Promise((resolve, reject) => {
    ...
    ff.on("close", (code) => {
      inflight.delete(key);
      if (code !== 0) return reject(new Error("Transcode failed"));
...
export async function handleManifest(url) {
  const text = await fetchText(url);
  return rewritePlaylist(text, url);
}
  ...
  return async (req, res, next) => {
    ...
    try {
      if (pathname === "/hls/manifest") {
        ...
        res.end(await handleManifest(target));
        return;
      }
